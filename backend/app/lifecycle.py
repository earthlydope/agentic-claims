...
stages that were missing and that a claim genuinely cannot finish without:
  ...
  * the total-loss test — repair cost against replacement value, which is the assessor's
    call and not the handler's (AKB-§11.2)
...
STAGES: tuple[Stage, ...] = (
    ...
    Stage(4, "triage", "Triage", LANE_PLATFORM, "claims_handler",
    ...
    Stage(5, "coverage", "Coverage", LANE_PLATFORM, "claims_handler",
          agent="CoverageAgent",
          summary="The policy position on the date of loss, with the clause relied on.",
          exceptions=("no authoritative clause — the agent abstains and refers",)),
    ...
    Stage(9, "risk", "Risk screening", LANE_PLATFORM, "siu_investigator",
    ...
    Stage(10, "decision", "Decision", LANE_PLATFORM, "claims_handler",
          agent="DecisionAgent",
    ...
    Stage(12, "approval", "Human approval", LANE_PEOPLE, "team_leader",
    ...
    Stage(13, "settle", "Settlement", LANE_PLATFORM, "claims_handler", pillar=3,
    ...
    Stage(14, "recovery", "Recovery", LANE_PEOPLE, "claims_handler",
          agent="RecoveryAgent",
    ...
    Stage(15, "close", "Close & learn", LANE_PLATFORM, "compliance_officer",
...
STATUSES: tuple[ClaimStatus, ...] = (
    ...
    ClaimStatus("in_review", "With a person", "approval", "warn",
                description="A deterministic check stopped it, or the agent asked for review."),
...
def status_meta(key: str | None) -> dict[str, Any]:
    ...
    if status is None:
        return {"key": key or "unknown", "label": (key or "unknown").replace("_", " "),
